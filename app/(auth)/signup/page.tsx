...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await signup(formData, callbackUrl)
      if (result?.error) {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <motion.div
        className="w-full max-w-lg glass-panel p-10 rounded-[2.5rem] relative overflow-hidden"
      ...
      >
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold mb-3 tracking-tight text-white">Join the Elite</h1>
          <p className="text-white/40 font-medium">Start your 14-day premium trial today.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-white/60 uppercase tracking-widest ml-1">Confirm</label>
              <Input
                type="password"
          ...
          <div className="flex items-start gap-3 ml-1 pt-2">
            <input 
              type="checkbox" 
              id="terms" 
              className="w-5 h-5 rounded-lg border-white/10 bg-white/5 checked:bg-primary transition-all cursor-pointer mt-0.5" 
              required
            />
            <label htmlFor="terms" className="text-sm font-medium text-white/40 cursor-pointer select-none leading-relaxed">
              I agree to the <Link href="/terms" className="text-primary hover:text-primary/80 font-bold transition-colors">Terms of Service</Link> and <Link href="/privacy" className="text-primary hover:text-primary/80 font-bold transition-colors">Privacy Policy</Link>
            </label>
          </div>
        ...
        <div className="relative my-10">
          ...
          <div className="relative flex justify-center text-xs font-bold uppercase tracking-widest">
            <span className="px-4 bg-[#0a0a0a] text-white/20">Or Quick Sign Up With</span>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-6 mt-10">
           {["No Credit Card", "14-Day Free", "Cancel Anytime"].map((badge) => (
             <div key={badge} className="flex items-center gap-1.5">
               <Check className="w-3.5 h-3.5 text-primary" />
               <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{badge}</span>
             </div>
           ))}
        </div>
      </motion.div>
